...
TOPIC_CONFIGURE_NETWORK_WIFI_INFO: Final = "/hkmw/robo/configure_network/wifi_info"
DETECT_FLAG: Final = 0xFFFEA1B2
...
def on_conn_state_changed(conn_state: ConnectionState, e: Exception):
   ...
   if conn_state == ConnectionState.CONNECTED:
      connection.subscribe(TOPIC_CONFIGURE_NETWORK_WIFI_INFO, Method.REQUEST, on_data_arrived)
...
if __name__ == '__main__':
   detector.start_discover(
      config_props={
         ...
         PropKeys.PROP_BROADCAST_PORT: 13000,
      },
   ...
   while True:
      ...
      if connection.get_state() == ConnectionState.CONNECTED:
         print('connected')
         #  connection.publish(TOPIC_CONFIGURE_NETWORK_WIFI_INFO, Method.REQUEST,
